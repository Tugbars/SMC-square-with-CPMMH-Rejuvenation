...
#pragma unroll
    for (int k = 0; k < OCSN_K; k++)
    {
        ...
        log_alpha_tilde[k] = d_OCSN_LOG_WEIGHTS[k] - 0.5f * __logf(S[k]) - 0.5f * innov * innov / S[k];
    ...
    float ess_threshold_inner)
{
    ...
    if (theta_idx >= N_theta || inner_idx >= N_inner)
        return;
    ...
    int N_theta)
{
    ...
    if (idx < N_theta)
        s_cumsum[idx] = particles.weight[idx];
    __syncthreads();

    if (idx == 0)
    {
        for (int i = 1; i < N_theta; i++)
            s_cumsum[i] += s_cumsum[i - 1];
    ...
    int N_theta, int N_inner)
{
    ...
    if (inner_idx < N_inner)
    {
        ...
        dst.rng_states[dst_idx] = src.rng_states[src_idx];
    }
}
...
SMC2StateCUDA *smc2_cuda_alloc(int N_theta, int N_inner)
{
    ...
    /* Default prior */
    state->prior.rho_mean = 0.95f;
    state->prior.rho_std = 0.02f;
    state->prior.sigma_z_mean = 0.1f;
    state->prior.sigma_z_std = 0.05f;
    state->prior.mu_base_mean = -1.0f;
    state->prior.mu_base_std = 0.5f;
    state->prior.mu_scale_mean = 0.5f;
    state->prior.mu_scale_std = 0.3f;
    state->prior.mu_rate_mean = 1.0f;
    state->prior.mu_rate_std = 0.5f;
    state->prior.sigma_base_mean = 0.15f;
    state->prior.sigma_base_std = 0.05f;
    state->prior.sigma_scale_mean = 0.1f;
    state->prior.sigma_scale_std = 0.05f;
    state->prior.sigma_rate_mean = 1.0f;
    state->prior.sigma_rate_std = 0.5f;

    /* Default bounds */
    state->bounds.rho_min = 0.8f;
    state->bounds.rho_max = 0.999f;
    state->bounds.sigma_z_min = 0.01f;
    state->bounds.sigma_z_max = 0.5f;
    state->bounds.mu_base_min = -5.0f;
    state->bounds.mu_base_max = 2.0f;
    state->bounds.mu_scale_min = -1.0f;
    state->bounds.mu_scale_max = 2.0f;
    state->bounds.mu_rate_min = 0.1f;
    state->bounds.mu_rate_max = 5.0f;
    state->bounds.sigma_base_min = 0.01f;
    state->bounds.sigma_base_max = 0.5f;
    state->bounds.sigma_scale_min = -0.2f;
    state->bounds.sigma_scale_max = 0.5f;
    state->bounds.sigma_rate_min = 0.1f;
    state->bounds.sigma_rate_max = 5.0f;
